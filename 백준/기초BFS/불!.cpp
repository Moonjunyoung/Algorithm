...
// //코드는 다이해됬음 
// 1. 불의 전파시간을 bfs 로 돌려서 초당 불의위치를 만듬
// 2. 그다음 지훈이의 bfs를돌리는데 이떄 방문하려는 지점이 불이 도착하는
//  시간보다 작아야지 도착이가능그떄 지훈이를 큐에넣어줌
// 3. 지훈이가 맵을 벗어난다면 즉 탈출이 가능한것이므로 해당 좌표일떄 지훈이의 현재위치값을 반환해줌
// 4. 맵을 벗어나지못한경우엔 큐가 빈경우일수밖에없음
